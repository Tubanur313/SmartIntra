...
namespace SmartIntranet.Web.Controllers
{
    public class WorkGraphicController : BaseIdentityController
    {
        ...
        {
            ViewBag.years = _mapper.Map<ICollection<NonWorkingYear>>(await _nonWorkingYearService.GetAllAsync(x => x.IsDeleted  == false));
        ...
        public async Task<IActionResult> Add(WorkGraphicAddDto model)
        {
            ...
            else
            {
                ...
                await _workGraphicService.AddAsync(_mapper.Map<WorkGraphic>(model));
        ...
        public async Task<IActionResult> Update(int id)
        {
            ...
            var listModel = _mapper.Map<WorkGraphicUpdateDto>(await _workGraphicService.FindByIdAsync(id));
            if (listModel == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> Update(WorkGraphicUpdateDto model)
        {
            ...
            else
            {
                ...
                await _workGraphicService.UpdateAsync(_mapper.Map<WorkGraphic>(model));
                return RedirectToAction("List");
            }
        }
        ...
        {
            var transactionModel = _mapper.Map<WorkGraphicListDto>(await _workGraphicService.FindByIdAsync(id));
            var current = GetSignInUserId();
            transactionModel.DeleteDate = DateTime.UtcNow.AddHours(4);
            transactionModel.DeleteByUserId = current;
            transactionModel.IsDeleted = true;
            await _workGraphicService.UpdateAsync(_mapper.Map<WorkGraphic>(transactionModel));
            return Ok();
