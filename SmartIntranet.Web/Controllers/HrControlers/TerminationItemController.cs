    ...
    public class TerminationItemController : BaseIdentityController
    {
        ...
        public async Task<IActionResult> Add(TerminationItemAddDto model)
        {
            ...
            else
            {
                ...
                await _terminationService.AddAsync(_mapper.Map<TerminationItem>(model));
        ...
        {
            var listModel = _mapper.Map<TerminationItemUpdateDto>(await _terminationService.FindByIdAsync(id));
            if (listModel == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> Update(TerminationItemUpdateDto model)
        {
            ...
            else
            {
                ...
                await _terminationService.UpdateAsync(_mapper.Map<TerminationItem>(model));
                return RedirectToAction("List");
            }
        }
        ...
        {
            var transactionModel = _mapper.Map<TerminationItemListDto>(await _terminationService.FindByIdAsync(id));
            var current = GetSignInUserId();
            transactionModel.DeleteDate = DateTime.UtcNow.AddHours(4);
            transactionModel.DeleteByUserId = current;
            transactionModel.IsDeleted = true;
            await _terminationService.UpdateAsync(_mapper.Map<TerminationItem>(transactionModel));
            return Ok();
        }
    }
}
